...
export const Challenge = ({
  ...
  const [fermiQuestionNum, setFermiQuestionNum] = useState(
    challenge.fermiQuestions &&
      challenge.fermiQuestions.findIndex(
        (question) =>
          !question.teamAnswers.some((answer) => answer.teamId === teamId)
      )
  )
  const [aboveBelowQuestionNum, setAboveBelowQuestionNum] = useState(
  ...
  const accumulatePoints = (
    acc: number,
    question: { teamAnswers: { teamId: string; score: number }[] }
  ) => {
    const teamAnswer = question.teamAnswers.find(
      (answer) => answer.teamId === teamId
    )
    return teamAnswer ? acc + teamAnswer.score : acc
  }
  const [fermiScore, setFermiScore] = useState(
    challenge.fermiQuestions.reduce(accumulatePoints, 0)
  )
  const [aboveBelowScore, setAboveBelowScore] = useState(
    challenge.aboveBelowQuestions.reduce(accumulatePoints, 0)
  )

  const [questionComplete, setQuestionComplete] = useState(false)
  const [introsShown, setIntrosShown] = useState({
    fermi: false,
    aboveBelow: false,
  })
  ...
  const fermiComplete =
    !challenge.fermiQuestions ||
    fermiQuestionNum === -1 ||
    fermiQuestionNum >= challenge.fermiQuestions.length
  const aboveBelowComplete =
    !challenge.aboveBelowQuestions ||
    aboveBelowQuestionNum === -1 ||
    aboveBelowQuestionNum >= challenge.aboveBelowQuestions.length
  const challengeComplete = fermiComplete && aboveBelowComplete

  const fermiQuestion = fermiComplete
    ? undefined
    : challenge.fermiQuestions[fermiQuestionNum]
  ...
  return (
    <div className="px-4 py-6 grow">
      <div className="py-8 max-w-prose mx-auto">
          ...
          <div className="prose">
            ...
            {!challengeComplete && (
              <h4 className="pt-8 text-gray-500">
                ...
                /
                {challenge.fermiQuestions.length +
                  challenge.aboveBelowQuestions.length}
              </h4>
            )}
          </div>
          <div className="prose">
            <h4 className="my-0 text-gray-700">
              {challenge.teams.find((t) => t.id === teamId)?.name}
            </h4>
            ...
            {fermiComplete && (
              <>
                <h2 className="mt-2 mb-0 text-gray-500 text-[16px]">
                  Round 2{" "}
                </h2>
                <h2 className="my-0">
            ...
            <>
              {!introsShown.fermi && (
                <div className="prose bg-indigo-100 rounded-md p-8 my-6">
                  ...
                  <p className="my-0">
                    {"Estimate a lower and upper bound for the question below."}
                  </p>
                  <p className="my-0">
                    {
                      "A narrower interval gets you more points, if you're right."
                    }
                  </p>
                  <p className="my-0">
                    {
                      "But if you're wrong, a narrower interval loses you more points!"
                    }
                  </p>
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      onClick={() =>
                        setIntrosShown({ ...introsShown, fermi: true })
                      }
                    >
                      {"Got it"}
                    </button>
              ...
              <>
                {!introsShown.aboveBelow && (
                  <div className="prose bg-indigo-100 rounded-md p-8 my-6">
                    ...
                    <p className="my-0">
                      {
                        "Estimate if the real quantity is above or below the amount below."
                      }
                    </p>
                    <p className="my-0">
                      {
                        "Higher confidence gets you more points, if you're right."
                      }
                    </p>
                    <p className="my-0">
                      {
                        "But if you're wrong, higher confidence loses you more points!"
                      }
                    </p>
                    <div className="pt-2">
                      <button
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
                        onClick={() =>
                          setIntrosShown({ ...introsShown, aboveBelow: true })
                        }
                      >
                        {"Got it"}
                      </button>
                ...
                <AboveBelowForm
                  ...
                  addToScore={(newPoints) =>
                    setAboveBelowScore(aboveBelowScore + newPoints)
                  }
                  setQuestionComplete={setQuestionComplete}
                  nextQuestion={() => {
        ...
        {questionComplete && !challengeComplete && (
          <ChallengeLeaderboard
            challengeId={challenge.id}
            teamId={teamId}
            latestQuestion={{
              type: fermiComplete ? "aboveBelow" : "fermi",
              indexWithinType: fermiComplete
                ? aboveBelowQuestionNum
                : fermiQuestionNum,
            }}
          />
